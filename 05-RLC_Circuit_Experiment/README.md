...
## Circuit Description
The main goal of the circuit design was to develop a method for studying the charging behavior of a capacitor. To solve for the capacitor's charging equation, two initial conditions are required: the capacitor's initial voltage (or equivalently, its initial charge) and the initial current provided by the inductor. Determining the initial current was the main challenge, as we had to figure out how it could be obtained from the circuit configuration. The following circuit description explains our thought process and the reasoning behind how we designed the circuit to determine these unknown constants.
